...
export const cartReducer = (previousState = INITIAL_STATE, action) => {
  switch (action.type) {
    case "ADD_PRODUCT": {
      ...
      const productCart = cart[0].find(
        (item) => item.id === product.id
      );

      if (productCart) {
        let index = cart[0].findIndex(
          (item) => item.id === product.id
        );
        cart[0][index].count += 1;
      }
      else {
        cart[0].push(
          {
    ...
    case "DELETE_PRODUCT": {
      ...
      if(cart[0].length !== 0)
      {
        if (cart[0][index].count === 1 && cart[0].length) {
    ...
    case "SHOW_CART": {
      ...
      return { ...previousState, showCart: action.payload };
    }
    default:
